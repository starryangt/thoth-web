module Epub
    ...
    let CreateEpub (book : Book) (UUID : string) =
        try
        ...
        let ToData (path : string) =
            let data_dir = "./data/" + UUID
            IO.Path.Combine(data_dir, path)
        ...
        Directory.CreateDirectory (ToData "") |> ignore
        CheckAndDelete (ToData ((safePath book.metadata.["title"]) + ".epub"))

        //Create new zip
        let newZip = ZipFile.Open(ToData ((safePath book.metadata.["title"]) + ".epub"), ZipArchiveMode.Create)
        ...
        book.chapters |> List.iter (fun (x : Chapter) ->
            newZip.CreateEntryFromFile((x.directory), "OEBPS/" + x.src) |> ignore)
