...
Elm.Main.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm),
   $Task = Elm.Task.make(_elm);
   var places = $Json$Decode.succeed("asdlfasd");
   var listToString = function (list) {
      return function () {
         var urls = A2($List.map,
         function (x) {
         ...
         0,
         $Json$Encode.list(urls));
      }();
   };
   var sendUrls = function (list) {
      return A3($Http.post,
      $Json$Decode.string,
      "http://muchnothing.com/thoth",
      $Http.string(listToString(list)));
   };
   var errorView = function (model) {
      return A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([$Html.text("Something went wrong. You could try again, but it probably won\'t work again.")]));
   };
   var presentUrlView = function (model) {
      return A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([A2($Html.a,
                   _L.fromArray([$Html$Attributes.href(A2($Basics._op["++"],
                   "http://muchnothing.com",
                   model.text))]),
                   _L.fromArray([$Html.text("Download Link")]))
                   ,A2($Html.br,
                   _L.fromArray([]),
                   _L.fromArray([]))
                   ,$Html.text("Download links are expunged after 10 minutes.")]));
   };
   var downloadView = function (model) {
      return A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([$Html.text("Downloading...")]));
   };
   var urlsToString = function (urls) {
      return A2($List.map,
      function (x) {
         return x.text;
      },
   ...
   var strictToHtml = function (bool) {
      return function () {
         switch (bool)
         {case false:
            return $Html.text("Strict Mode: ON");
            case true:
            return $Html.text("Strict Mode: OFF");}
         _U.badCase($moduleName,
   ...
   value) {
      ...
      function (x) {
         return _U.eq(x,
         13) ? $Result.Ok({ctor: "_Tuple0"}) : $Result.Err("no");
      }),
      function (_v1) {
         return function () {
            return A2($Signal.message,
   ...
   var SetStrict = {ctor: "SetStrict"};
   var GotPost = function (a) {
      return {ctor: "GotPost"
             ,_0: a};
   };
   var SendUrls = function (a) {
      return {ctor: "SendUrls"
             ,_0: a};
   };
   ...
   var NewUrl = {ctor: "NewUrl"};
   var SetUrls = F2(function (a,
   b) {
      return {ctor: "SetUrls"
             ,_0: a
             ,_1: b};
   });
   var SetCover = function (a) {
      return {ctor: "SetCover"
             ,_0: a};
   };
   var SetAuthor = function (a) {
      return {ctor: "SetAuthor"
             ,_0: a};
   };
   var SetTitle = function (a) {
      return {ctor: "SetTitle"
             ,_0: a};
   };
   var SetString = function (a) {
      return {ctor: "SetString"
             ,_0: a};
   };
   var NoOp = {ctor: "NoOp"};
   var actions = $Signal.mailbox(NoOp);
   var requestBoom = A2($Signal.send,
   actions.address,
   Explode);
   var urlToHtml = function (url) {
      return A2($Html.input,
      _L.fromArray([$Html$Attributes.$class("input")
                   ,$Html$Attributes.value(url.text)
                   ,A3($Html$Events.on,
                   "input",
                   $Html$Events.targetValue,
                   function ($) {
                      return $Signal.message(actions.address)(SetUrls(url.id)($));
                   })]),
      _L.fromArray([]));
   };
   var urlsToHtml = function (urls) {
      return A2($List.map,
      urlToHtml,
      urls);
   };
   var inputView = function (model) {
      ...
      _L.fromArray([A2($Html.input,
                   _L.fromArray([$Html$Attributes.$class("metainput")
                                ...
                                $Html$Events.targetValue,
                                function ($) {
                                   return $Signal.message(actions.address)(SetTitle($));
                                })]),
                   _L.fromArray([]))
                   ,A2($Html.input,
                   _L.fromArray([$Html$Attributes.$class("metainput")
                                ...
                                $Html$Events.targetValue,
                                function ($) {
                                   return $Signal.message(actions.address)(SetAuthor($));
                                })]),
                   _L.fromArray([]))
                   ,A2($Html.input,
                   _L.fromArray([$Html$Attributes.$class("metainput")
                                ,$Html$Attributes.placeholder("Cover URL")
                                ,A3($Html$Events.on,
                                "input",
                                $Html$Events.targetValue,
                                function ($) {
                                   return $Signal.message(actions.address)(SetCover($));
                                })]),
                   _L.fromArray([]))
                   ,A2($Html.br,
                   _L.fromArray([]),
                   ...
                   _L.fromArray([]),
                   _L.fromArray([]))
                   ,$Html.text("Strictness will output all content as <p> tags. Turn it on if the website is poorly formatted or if your e-reader doesn\'t like non-XHTMl spec stuff. Basically, if the epub produced doesn\'t open, try it with strict on.")
                   ,A2($Html.input,
                   _L.fromArray([$Html$Attributes.$class("input")
                                ,$Html$Attributes.placeholder("New Url")
                                ,A2(onEnter,
                                actions.address,
                                NewUrl)
                                ,A3($Html$Events.on,
                                ...
                                $Html$Events.targetValue,
                                function ($) {
                                   return $Signal.message(actions.address)(SetString($));
                                })
                   ...
                   _L.fromArray([]))
                   ,A2($Html.div,
                   _L.fromArray([$Html$Attributes.$class("url-form")]),
                   $List.reverse(urlsToHtml(model.urls)))
                   ,A2($Html.button,
                   _L.fromArray([A2($Html$Events.onClick,
                   actions.address,
                   SendUrls(A2($Basics._op["++"],
                   _L.fromArray([model.title
                                ,model.author
                                ,model.cover
                                ,boolToString(model.strict)]),
                   urlsToString(model.urls))))]),
   ...
   var view = function (model) {
      return function () {
         ...
         switch (_v4.ctor)
         {case "Download":
            return downloadView(model);
            case "Error":
            return errorView(model);
            case "Input":
            return inputView(model);
            case "PresentUrl":
            return presentUrlView(model);}
         _U.badCase($moduleName,
   ...
   var PresentUrl = {ctor: "PresentUrl"};
   var Download = {ctor: "Download"};
      ...
      return function () {
         ...
         {case "Explode":
            ...
            return _U.replace([["status"
              ...
              model);
            case "SetAuthor":
            return _U.replace([["author"
                               ,action._0]],
              model);
            case "SetCover":
            return _U.replace([["cover"
                               ,action._0]],
              model);
            case "SetList":
            return _U.replace([["text"
              ...
              model);
            case "SetStrict":
            return _U.replace([["strict"
                               ,$Basics.not(model.strict)]],
              model);
            case "SetString":
            return _U.replace([["text"
                               ,action._0]],
            ...
            return _U.replace([["urls"
              ...
              model);}
         _U.badCase($moduleName,
         "between lines 74 and 113");
      }();
   });
   var updateModel = A3($Signal.foldp,
   update,
   initialModel,
   actions.signal);
   ...
   view,
   updateModel);
   var Url = F2(function (a,b) {
      return {_: {},id: b,text: a};
   });
   _elm.Main.values = {_op: _op
                      ...
                      ,SetStrict: SetStrict
                      ,update: update
                      ,requestBoom: requestBoom
                      ,boolToString: boolToString
                      ,onEnter: onEnter
                      ,urlToHtml: urlToHtml
                      ,urlsToHtml: urlsToHtml
                      ,strictToHtml: strictToHtml
                      ,inputView: inputView
                      ,urlsToString: urlsToString
                      ,downloadView: downloadView
                      ,presentUrlView: presentUrlView
                      ,errorView: errorView
                      ,view: view
                      ,actions: actions
